...
const MIN_COMMENTARY_GAP_MS = 15 * 60 * 1000; // minimum 15 min between Claude commentary calls
...
export async function generateCommentary(
  ...
  // ── Claude Cost Control ────────────────────────────────────────────────
  // Only let Claude speak on strong setups or truly urgent triggers.
  const hasStrongSetup = currentScore >= 70;
  const isHighUrgency = trigger.urgency === "high";
  const hasOrderFlow =
    ((latest as any).delta !== null && (latest as any).delta !== undefined) ||
    (latest as any).absorptionBull === 1 ||
    (latest as any).absorptionBear === 1 ||
    (latest as any).imbalanceBull === 1 ||
    (latest as any).imbalanceBear === 1;

  // Skip Claude for low-score / low-urgency noise.
  if (!hasStrongSetup && !isHighUrgency) return;

  // If the score is weak and Sierra order flow is not present, skip completely.
...
// ── 15-Minute Market Pulse ─────────────────────────────────────────────────────
// Fires every 15 minutes during market hours regardless of signal events
...
export function startPulse() {
  ...
  console.log("[Pulse] 15-minute market pulse started");
}
